...
class CubicSparsityScheduler(SparsityScheduler):
    def __init__(self, initial_sparsity: float, final_sparsity: float, 
                 total_train_steps: int, pruning_start_step: int, 
                 pruning_end_step: int, pruning_interval: int):
...
class StepSparsityScheduler(SparsityScheduler):
    def __init__(self, final_sparsity: float, 
                 total_train_steps: int, pruning_start_step: int, 
                 pruning_end_step: int, sparsity_delta: float):
        ...
        self.num_pruning_steps = self.num_pruning_steps = int((self.final_sparsity - self.initial_sparsity) / self.sparsity_delta)
        self.pruning_interval = int((pruning_end_step - pruning_start_step) / self.num_pruning_steps)
